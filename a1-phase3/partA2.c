...
PROCESS ChildThreadFunction(void *arg)
{
    ...
    for (i = 1; i <= incoming_size; i++)
    {
        if (keep_running)
        {
            current_thread = thread_index; /* Set before each Square() call */
            Square(i);
        }
...
PROCESS ParentThreadFunction(void *arg)
{
    ...
    /* mem allocs */
    child_thread = (PID*)malloc(number_of_threads * sizeof(PID));
...
int mainp(int argc, char *argv[])
{
    ...
    int *input_args = malloc(3 * sizeof(int));
    ...
    usleep((deadline + 1) * 1000000); /* same as Sleep(6) - 6 ticks */
